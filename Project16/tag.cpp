...
namespace tag 
{
    void addTagsToTask(pqxx::work& txn, int task_id, const crow::json::rvalue& jsonData)
    {
        if (jsonData.has("tags"))
        {
            for (const auto& tag : jsonData["tags"])
            {
                ...
                // ѕровер€ем, существует ли тег, если нет, то добавл€ем его
                auto tagCheck = txn.exec_params("SELECT tag_id FROM tags WHERE tag_name = $1", tag_name);

                int tag_id;
                if (tagCheck.empty()) {
                    // ≈сли тег не существует, то создаем новый
    ...
    crow::response addTags(const crow::request& req, int task_id)
    {
        try
        {
            ...
            // ѕроверка, существует ли задача с task_id и принадлежит ли она пользователю
